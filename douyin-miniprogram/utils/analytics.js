...
function captureEntry(options = {}) {
  ...
  let channel;
  if (query.src !== undefined && query.src !== "") channel = normalizeChannel(query.src);
  else {
    try {
      channel = normalizeChannel(tt.getStorageSync(CHANNEL_KEY));
    } catch (error) {
      channel = "organic";
    }
  }
  try {
    tt.setStorageSync(CHANNEL_KEY, channel);
  } catch (error) {
    // 渠道统计失败不能影响小程序启动。
  }
  return channel;
...
function getChannel() {
  try {
    return normalizeChannel(tt.getStorageSync(CHANNEL_KEY));
  } catch (error) {
    return "organic";
  }
}
...
function report(eventName, data = {}) {
  ...
  if (!fields || typeof tt === "undefined" || typeof tt.reportAnalytics !== "function") return false;
  const payload = {
    channel: getChannel(),
  ...
  try {
    tt.reportAnalytics(eventName, payload);
    return true;
  } catch (error) {
    return false;
  }

...
function prepareShareImage(activityKey) {
  ...
  const manager = tt.getFileSystemManager();
...
function getCanvasNode(page) {
  if (!page || typeof tt === "undefined" || !tt.createSelectorQuery) return Promise.resolve(null);
...
function exportCanvas(canvas, page) {
  if (!tt.canvasToTempFilePath) return Promise.resolve("");
  return new Promise((resolve) => {

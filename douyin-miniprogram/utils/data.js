...
function normalizeCustomActivityConfig(config = {}) {
  ...
  const stamp = String(config.stamp || "").trim().slice(0, CUSTOM_ACTIVITY_LABEL_MAX_LENGTH) || label.slice(0, 1) || "+";
  return { label, stamp };
}
...
function getCustomActivity() {
  const saved = tt.getStorageSync(CUSTOM_ACTIVITY_KEY);
...
function saveCustomActivity(config) {
  const normalized = normalizeCustomActivityConfig(config);
  if (!normalized.label) {
    tt.removeStorageSync(CUSTOM_ACTIVITY_KEY);
    return getCustomActivity();
  }
  tt.setStorageSync(CUSTOM_ACTIVITY_KEY, normalized);
  return getCustomActivity();
}
...
function hasProfile() {
  const saved = tt.getStorageSync(PROFILE_KEY);
...
function saveProfile(profile) {
  ...
  tt.setStorageSync(PROFILE_KEY, normalized);
  return normalized;
}
...
function getActiveTimer() {
  const saved = tt.getStorageSync(ACTIVE_TIMER_KEY);
  const normalized = saved && typeof saved === "object" ? normalizeActiveTimer(saved) : null;
  if (!normalized && saved) tt.removeStorageSync(ACTIVE_TIMER_KEY);
  return normalized;
}
...
function saveActiveTimer(timer) {
  ...
  try {
    tt.setStorageSync(ACTIVE_TIMER_KEY, normalized);
    return normalized;
  } catch (error) {
    return null;
  }
...
function clearActiveTimer() {
  tt.removeStorageSync(ACTIVE_TIMER_KEY);
}

function getGoal() {
  const saved = tt.getStorageSync(GOAL_KEY);
...
function saveGoal(goal) {
  if (typeof goal === "string") {
    tt.setStorageSync(GOAL_KEY, goal);
    return getGoal();
  }
  const normalized = normalizeGoal(goal);
  tt.setStorageSync(GOAL_KEY, normalized);
  return normalized;
}
...
function clearAllData() {
  tt.removeStorageSync(PROFILE_KEY);
  tt.removeStorageSync(SESSIONS_KEY);
  tt.removeStorageSync(GOAL_KEY);
  tt.removeStorageSync(CUSTOM_ACTIVITY_KEY);
  tt.removeStorageSync(LEGACY_LEDGER_KEY);
  tt.removeStorageSync(ACTIVE_TIMER_KEY);
  tt.removeStorageSync(ENTRY_CHANNEL_KEY);
}
